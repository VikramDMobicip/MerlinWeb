...
public class BlogPage extends BrowserSetupClass {
	 ...
	 * spark = new
	 * ExtentSparkReporter("TestReports\\PreLoginTest\\BlogTest\\BlogReport.html");
	...
	public void a1_BlogPage() throws InterruptedException {
		...
		new WebDriverWait(webDriver, Duration.ofSeconds(20)).until(
				ExpectedConditions.visibilityOfElementLocated(By.xpath("//ul[@class='footer_ul']//a[text()='Blog']")));
		webDriver.findElement(By.xpath("//ul[@class='footer_ul']//a[text()='Blog']")).click();
		new WebDriverWait(webDriver, Duration.ofSeconds(20))
				.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//a[@title='Blog']")));
	...
	public void a2_Section1() throws InterruptedException {
		...
		boolean a = true;
		try {
			webDriver.findElement(By.xpath("//div[@class='banner_blog_info']/a/div[1]"));
		} catch (NoSuchElementException e) {
			a = false;
		...
		a = true;
		try {
			webDriver.findElement(By.xpath("//div[@class='banner_blog_info']/a/div[2]"));
		} catch (NoSuchElementException e) {
			a = false;
		...
		a = true;
		try {
			webDriver.findElement(By.xpath("//div[@class='banner_blog_container']/a[1]/div/img"));
		} catch (NoSuchElementException e) {
			a = false;
	...
	public void a4_Section2() throws InterruptedException {
		...
		boolean a = true;
		try {
			webDriver.findElement(By.xpath("//*[text()='Popular Posts']"));
		} catch (NoSuchElementException e) {
			a = false;
		...
		a = true;
		try {
			webDriver.findElement(By.xpath("//div[@class='blogs_container']/article[11]")).click();
		} catch (NoSuchElementException e) {
			a = false;
			Assert.fail("Article redirects failed");
		}
		if (a == true) {
			test3.pass("Article redirects works fine");
		}
		TimeUnit.SECONDS.sleep(2);
		webDriver.navigate().back();
		TimeUnit.SECONDS.sleep(2);
	...
	public void a5_Section2() throws InterruptedException {
		ExtentTest test4 = extentreport.createTest("Pagination",
				"To check the Prelogin Blog web page section 2 - Pagination");
		...
		try {
			...
			a = false;
			Assert.fail("Pagination is not present");
		}
		if (a == true) {
			test4.pass("Pagination is present");
		}
		...
		try {
			...
			a = false;
			Assert.fail("Random page redirect works fine");
		}
		if (a == true) {
			test4.pass("Random page redirect works fine");
		}
		...
		try {
			...
			a = false;
			Assert.fail("Arrow redirect works fine");
		}
		if (a == true) {
			test4.pass("Arrow redirect works fine");
		}
	...
	public void a6_section3() throws InterruptedException {
		...
		try {
			...
			a = false;
			Assert.fail("Blog page - Build better is not present");
		}
		if (a == true) {
			test5.pass("Blog page - Build better is present");
		}
		...
		try {
			...
			a = false;
			Assert.fail("Blog page - Footer is not present");
		}
		if (a == true) {
			test5.pass("Blog page - Footer is present");
		}
		try {
			TimeUnit.SECONDS.sleep(3);
